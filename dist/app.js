...
function initTables() {
    var students = retrieveTable('students');
    var htmlString = '<thead><tr><th>${clr(student.id)}</th><th>${clr(student.name)}</th><th>${clr(student.surname)}</th><th>${clr(student.teamId)}</th><th>${clr(student.tasksIds)}</th>><th>${clr(student.prior)}</th></tr></thead><tbody>';
